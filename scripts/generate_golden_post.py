...
class GoldenGen:
    ...
    def stock(self):
        ...
        y=260; t=f"{self.data.get('stock_name','')} — {self.data.get('stock_symbol','')}"
        ...
        if s:
            sb=self.draw.textbbox((0,0),f"القطاع: {s}",font=self.f["label"])
            self.draw.text(((SZ[0]-(sb[2]-sb[0]))//2, y+60), f"🏢 القطاع: {s}", font=self.f["label"], fill=COLORS["gray"])
    def row(self,l,v,c,ic="",y=None):
        ...
        self.draw.text((PAD+30,y+20),f"{ic} {l}",font=self.f["label"],fill=COLORS["gray"])
