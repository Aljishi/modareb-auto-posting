...
class RasedNormalGenerator:
    ...
    def draw_header(self):
        txt = f"{BRANDING['name']} | إشارة اليوم"
    ...
    def draw_stock_info(self):
        ...
        y = 160
        title = f"{self.data.get('stock_name','')} — {self.data.get('stock_symbol','')}"
    ...
    def _draw_row(self, label, value, color, icon="", y=None):
        ...
        self.draw.text((PADDING+20, y+18), f"{icon} {label}", font=self.fonts["label"], fill=COLORS["gray"])
    ...
    def draw_analysis(self):
        ...
        if read:
            self.draw.text((PADDING, y), "📌 قراءة فنية:", font=self.fonts["label"], fill=COLORS["accent"])
            y += 32
            words = read.split(); line = ""
            for w in words:
                test = f"{line} {w}" if line else w
                if self.draw.textbbox((0,0), test, font=self.fonts["tiny"])[2] < IMG_SIZE[0]-PADDING*2:
                    line = test
                else:
                    if line:
                        self.draw.text((PADDING, y), f"• {line}", font=self.fonts["tiny"], fill=COLORS["gray"])
                        y += 26
                    line = w
            if line: self.draw.text((PADDING, y), f"• {line}", font=self.fonts["tiny"], fill=COLORS["gray"])

    def draw_footer(self):
        fy = IMG_SIZE[1] - 130
        self.draw.line([(PADDING, fy), (IMG_SIZE[0]-PADDING, fy)], fill=COLORS["border"], width=2)
        w = "⚠️ محتوى تعليمي وتحليلي فقط — لا يعد توصية استثمارية"
        ...
        wm = f"️ {BRANDING['name']} | {BRANDING['channel']}"
